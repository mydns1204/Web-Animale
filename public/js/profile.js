...
// 	if (urlParams.get("user")==1){

// 		document.getElementById('username').innerHTML = "Top";
//         document.getElementById('datadescription').innerHTML = "นอน เเล้วลุกขึ้น";
//         document.getElementById('datatack').innerHTML = "#นาก #แมว";
...
function pageLoad(){
    ...
    var postBut = document.getElementById('postfeed').onclick = submitpost;
	
	var username = getCookie('username');
    document.getElementById("username").innerHTML = username;
    descrip = getCookie('descript');
    document.getElementById("datadescription").innerHTML = descrip;
    tackdb = getCookie('tackD');
    document.getElementById("datatack").innerHTML = tackdb;
...
async function postfeed(data) {
    ...
    for (var i = postkeys.length - 1; i >= 0; i--) {
        ...
        gridallpost.className = "allboxpost";
...
const sendlikepost = (async(post_id) => {
    await fetch("/likepost", {
        ...
        console.log(response);
        response.json().then((data) => {
            post_id.lastChild.firstChild.src = "pic/like.png";
